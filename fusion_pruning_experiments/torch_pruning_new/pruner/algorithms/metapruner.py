        ...
        if self.global_pruning:
            ...
            for group in self.DG.get_all_groups(
                ...
                initial_total_channels += (
                    self.DG.get_out_channels(group[0][0].target.module) // ch_groups
                )
    ...
    def merge_bn(self):
        for _, m in self.model.named_modules():
            if isinstance(m, (torch.nn.BatchNorm2d, torch.nn.BatchNorm1d)):
                group = self.DG.get_pruning_group( m, function.prune_batchnorm_in_channels, idxs=[0])
                for dep, _ in group:
...
=======
                        conv_bias = dep.target.module.bias if dep.target.module.bias != None else 0.0
>>>>>>> ab47b5d (Improve dimensionality_preserving to be aligned)
                        ...
                        conv_beta = dep.source.module.bias#state_dict[f'bn1.bias'].double()

                        new_conv = (conv / conv_var[:,None,None,None]) * conv_gamma[:,None,None,None]
                        new_conv_bias = ((conv_bias - conv_mean)/conv_var)*conv_gamma + conv_beta

                        shape = conv_mean.shape[0]
                        dep.target.module.weight = torch.nn.Parameter(new_conv)
                        dep.target.module.bias = torch.nn.Parameter(new_conv_bias)
                        dep.source.module.running_mean = torch.full((shape,),0.0)
                        dep.source.module.running_var = torch.full((shape,),1.0)
                        dep.source.module.weight = torch.nn.Parameter(torch.full((shape,),1.0))
                        dep.source.module.bias = torch.nn.Parameter(torch.full((shape,),0.0))
    ...
    """def prune_local(self, layer_idx=None):
        if self.current_step > self.iterative_steps:
            return
        for group in self.DG.get_all_groups(
            ignored_layers=self.ignored_layers, root_module_types=self.root_module_types, layer_idx=layer_idx
        ):
            # check pruning rate
            if self._check_sparsity(group):
                module = group[0][0].target.module
