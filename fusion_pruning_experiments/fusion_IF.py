...
def compute_euclidian_distance_matrix(
    x, y, p=1, squared=True
):  # For some reason TA prefers squared to be Truez
...
def create_network_from_params(reference_model, param_list, gpu_id=-1, sparsity=1.0):
    ...
    assert len(list(model.parameters())) == len(
        param_list
    )  # Assumption: We are fusing into a model of same architecture
...
def create_network_from_parameters(reference_model, param_list, gpu_id=-1):
    ...
    def process_layer(layer, idx):
        if layer.is_conv:
            model_state_dict[keys[idx]] = layer.weight.view(
                layer.weight.shape[0],
                layer.weight.shape[1],
                model_state_dict[keys[idx]].shape[2],
                model_state_dict[keys[idx]].shape[3],
            )
        else:
...
def comparator(model0_args, model1_args):
    ...
    for _, ((_, layer0_weight), (_, layer1_weight)) in enumerate(
...
def IntraFusion_Clustering(
    network, sparsity=0.65, gpu_id=-1, metric=-1, eps=1e-7, resnet=False, output_dim=10
):
    ...
    for idx, (layer_name, layer_weight) in enumerate(network.named_parameters()):
        ...
        if len(layer_shape) > 2:
            ...
            layer_weight_data = layer_weight.data.view(
                layer_weight.shape[0], layer_weight.shape[1], -1
            )
        else:
            is_conv = False
        ...
        amount_pruned = (
            1 if amount_pruned == 0 else amount_pruned
        )  # Make sure we're not cancelling a whole layer
        ...
        if idx == 0:
            norm_layer = LA.norm(
                layer_weight_data.view(layer_weight_data.shape[0], -1), ord=metric, dim=1
            )
            ...
            if is_conv:
                ...
                M = get_ground_metric(layer_flattened, basis_weight, bias_weight, basis_bias)
            else:
            ...
            if is_conv:
                ...
                aligned_wt = torch.bmm(layer_weight_data.permute(2, 0, 1), T_var_conv).permute(
                    1, 2, 0
                )
                ...
                if layer_weight.data.shape[1] != T_var.shape[0]:
                    ...
                    layer_unflattened = layer_weight.data.view(
                        layer_weight.shape[0], T_var.shape[0], -1
                    ).permute(2, 0, 1)
            ...
            M = get_ground_metric(aligned_wt, basis_weight, bias_weight, basis_bias)
        ...
        histo = torch.from_numpy(
            get_histogram(layer_shape[0], indices=indices, add=False)
        )  # Every data point gets the same weight
        ...
        if remove_outlier:
            ...
            model = KMeans(n_clusters=amount_pruned, random_state=0).fit(
                aligned_wt.view(aligned_wt.shape[0], -1).numpy(), sample_weight=histo
            )
            labels = model.labels_
            centroids = model.cluster_centers_

            M = get_ground_metric(
                torch.from_numpy(centroids), aligned_wt.view(aligned_wt.shape[0], -1), None, None
            )
            ...
            indice_outlier = (
                indice_outlier[:amount_pruned]
                if aligned_wt.shape[0] - indice_outlier.shape[0] < amount_pruned
                else indice_outlier
            )
        ...
        geometric_fc = (
            torch.matmul(T_var.t(), aligned_wt.contiguous().view(aligned_wt.shape[0], -1))
            if idx != num_layers - 1
            else aligned_wt
        )
        ...
        if is_conv:
            ...
            geometric_fc = geometric_fc.view(
                torch.Size(
                    [
                        geometric_fc.shape[0],
                        int(geometric_fc.shape[1] / (layer_shape[2] * layer_shape[3])),
                        layer_shape[2],
                        layer_shape[3],
                    ]
                )
        ...
        if bias:
            geometric_bias = (
                torch.matmul(T_var.t(), bias_weight.view(bias_weight.shape[0], -1)).flatten()
                if idx != num_layers - 1
                else bias_weight
            )
    ...
    return create_network_from_params(
        gpu_id=gpu_id,
        param_list=avg_aligned_layers,
        reference_model=get_model(model_name="vgg11", sparsity=1 - sparsity, output_dim=output_dim),
    )
...
def intrafusion_bn(
    ...
    if gpu_id != -1:
        t = t.cuda(gpu_id)
    fusion_layers_2 = preprocess_parameters(
        [t],
        resnet=resnet,
        intrafusion=True,
    ...
    fusion_layers = preprocess_parameters(
        [model],
        resnet=resnet,
        intrafusion=True,
        fusion_type=fusion_type,
    ...
    for idx in range(num_layers):
        ...
        M = [
            get_ground_metric(
                fusion_layer.create_comparison_vec()[0],
                basis_weight[0].view(basis_weight[0].shape[0], -1),
                None,
                None,
            )
        ]
        if len(comp_vec) > 1:
            for i in range(1, len(comp_vec)):
                M.append(
                    get_ground_metric(
                        comp_vec[i], basis_weight[i].view(basis_weight[i].shape[0], -1), None, None
                    )
                )
            M = torch.stack(M)
        ...
        if meta_prune_type == MetaPruneType.DEFAULT:
            ...
            raise Exception(
                "Invalid prune type or invalid model name. Make sure you set resnet = True if you are pruning a resnet!"
            )
    ...
    return create_network_from_parameters(
        gpu_id=gpu_id, param_list=[layers[0] for layers in fusion_layers], reference_model=t
    )
...
def fusion_bn(
    ...
    networks_zip = sorted(
        list(zip(networks, accuracies, importance)), key=functools.cmp_to_key(comparator)
    )
        ...
        for idx_m, fusion_layer in enumerate(fusion_layers[idx][1:]):
            ...
            for i in range(0, len(comp_vec_ours)):
                M.append(
                    get_ground_metric(
                        comp_vec_ours[i],
                        comp_vec_avg[i].view(comp_vec_avg[i].shape[0], -1),
                        None,
                        None,
                    )
                )
            ...
            avg_layer.update_weights(
                fusion_layer, torch.sum(importance[:idx_model]), importance[idx_model]
            )
    ...
    return create_network_from_parameters(
        gpu_id=gpu_id,
        param_list=[layers[0] for layers in fusion_layers],
        reference_model=networks[0],
    )
...
def merge_conv_bn(conv, bn):
    ...
    merged_conv.weight = nn.Parameter(
        conv.weight
        * bn.weight[:, None, None, None]
        / torch.sqrt(bn.running_var + bn.eps)[:, None, None, None]
    )
    merged_conv.bias = nn.Parameter(
        (conv.bias if conv.bias is not None else torch.zeros_like(bn.running_mean))
        - bn.running_mean * bn.weight / torch.sqrt(bn.running_var + bn.eps)
        + bn.bias
    )
...
def fusion(networks, gpu_id=-1, accuracies=None, importance=None, eps=1e-7, resnet=False):
    ...
    for idx in range(num_layers):
        ...
        if idx != num_layers - 1:
            bias = (
                True
                if "bias"
                in next(itertools.islice(networks[0].named_parameters(), idx + 1, None))[0]
                else False
            )
            avg_bias_weight = (
                next(itertools.islice(networks[0].named_parameters(), idx + 1, None))[1]
                if bias
                else None
            )
        else:
        ...
        for idx_model in range(
            1, num_models
        ):  # We skip the first model because we align our models according to the first model
            T_var = T_var_list[idx_model]
            skip_T_var = skip_T_var_list[idx_model]
            skip_T_var_idx = skip_T_var_idx_list[idx_model]
            residual_T_var = residual_T_var_list[idx_model]
            ...
            bias_weight = (
                next(itertools.islice(networks[idx_model].named_parameters(), idx + 1, None))[1]
                if bias
                else None
            )
            ...
            if idx == 0:
                if is_conv:
                    ...
                    M = get_ground_metric(
                        layer_flattened, avg_layer_flattened, bias_weight, avg_bias_weight
                    )
                else:
                ...
                if is_conv:
                    ...
                    if layer.data.shape[1] != T_var.shape[0]:
                        # Handles the switch from convolutional layers to fc layers
                        layer_unflattened = layer.data.view(
                            layer.shape[0], T_var.shape[0], -1
                        ).permute(2, 0, 1)
                        aligned_wt = torch.bmm(
                            layer_unflattened,
                            T_var.unsqueeze(0).repeat(layer_unflattened.shape[0], 1, 1),
            ...
            avg_layer = (
                t_model * importance[idx_model]
                + avg_layer_data.view(avg_layer_data.shape[0], -1)
                * torch.sum(importance[:idx_model])
            ) / (torch.sum(importance[:idx_model]) + importance[idx_model])

            if is_conv and layer_shape != avg_layer.shape:
                avg_layer = avg_layer.view(layer_shape)

            if bias:
                t_bias_aligned = torch.matmul(
                    T_var.t(), bias_weight.view(bias_weight.shape[0], -1)
                ).flatten()
                avg_bias_weight = (
                    t_bias_aligned * importance[idx_model]
                    + avg_bias_weight * torch.sum(importance[:idx_model])
                ) / (torch.sum(importance[:idx_model]) + importance[idx_model])
    ...
    return create_network_from_params(
        gpu_id=gpu_id, param_list=avg_aligned_layers, reference_model=networks[0]
    )
...
def fusion_old(networks, args, gpu_id=-1, importance=[], eps=1e-7, resnet=False):
    ...
    for idx, ((layer0_name, fc_layer0_weight), (layer1_name, fc_layer1_weight)) in enumerate(
        zip(networks[0].named_parameters(), networks[1].named_parameters())
    ):
    ...
    for idx, ((layer0_name, fc_layer0_weight), (layer1_name, fc_layer1_weight)) in enumerate(
        ...
        # Check if this current layer has a bias
        if idx != num_layers - 1:
            next_layer0, next_layer1 = next(
                itertools.islice(networks[0].named_parameters(), idx + 1, None)
            ), next(itertools.islice(networks[1].named_parameters(), idx + 1, None))
        ...
        if idx == 0:
            if is_conv:
                ...
                M = get_ground_metric(
                    fc_layer0_flattened, fc_layer1_flattened, bias0_weight, bias1_weight
                )
            else:
                M = get_ground_metric(
                    fc_layer0_weight_data, fc_layer1_weight_data, bias0_weight, bias1_weight
                )
            ...
            if is_conv:
                ...
                aligned_wt = torch.bmm(fc_layer0_weight_data.permute(2, 0, 1), T_var_conv).permute(
                    1, 2, 0
                )
        ...
        # Averaging of aligned weights (Could potentially also favor one model over the other)
        geometric_fc = (
            t_fc0_model + fc_layer1_weight_data.view(fc_layer1_weight_data.shape[0], -1)
        ) / 2
        ...
        if bias:
            t_bias_aligned = torch.matmul(
                T_var.t(), bias0_weight.view(bias0_weight.shape[0], -1)
            ).flatten()
            # sum = torch.sum(T_var, 0)
            # t_bias_aligned/sum
    ...
    return create_network_from_params(
        args=args, param_list=avg_aligned_layers, reference_model=networks[1]
    )
...
def fusion_sidak_multimodel(
    networks, gpu_id=-1, accuracies=None, importance=None, eps=1e-7, resnet=False
):
    """
    ...
    for idx in range(num_layers):
        ...
        for idx_m, fusion_layer in enumerate(fusion_layers[idx][1:]):
            ...
            nu_cardinality = fusion_layer.weight.shape[
                0
            ]  # initialize with [0, 0, 0, ...] -> for number of nodes contained in the layer
            layer_shape = fusion_layer.weight.shape
            ...
            M = get_ground_metric(
                fusion_layer.create_comparison_vec(), avg_layer.create_comparison_vec(), None, None
            )
        ...
        for idx, to_include_layer in enumerate(aligned_fusions_to_average_over):
            avg_layer.update_weights(
                to_include_layer,
                aligned_fusions_to_average_over_importance[idx],
                aligned_fusions_to_average_over_own_importance[idx],
            )
