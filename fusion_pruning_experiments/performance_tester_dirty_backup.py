        ...
        for model_dict in experiment_params["models"]:
            ...
            if experiment_params["FaP"]:
                if experiment_params["num_epochs"] > 0:
                    ...
                    fap_model_path = f"{input_model_names[0][:-2]}_w{int(fusion_weights[0]*100)}_s{int(result['sparsity']*100)}_P{iterprune_text}_aT{int(num_epochs)}aFaT{int(num_epochs)}"
