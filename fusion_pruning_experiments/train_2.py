...
def main(
    ...
    def evaluate(model):
        ...
        return mean_test_accuracy, mean_test_loss
    
    mean_test_accuracy, mean_test_loss = evaluate(model) 

    if results_dict != None:
        results_dict["initial"] = {
                "test_accuracy": mean_test_accuracy.value(),
                "test_loss": mean_test_loss.value()}
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if tensorboard_obj is not None:
            ...
            tensorboard_obj.add_scalars(
                "train_loss/",
                {config["nick"]: mean_train_loss.value()},
                global_step=(epoch + 1),
            )
            tensorboard_obj.add_scalars(
                "train_accuracy_percent/",
                {config["nick"]: mean_train_accuracy.value() * 100},
                global_step=(epoch + 1),
            )
            tensorboard_obj.add_scalars(
                "test_loss/",
                {config["nick"]: mean_test_loss.value()},
                global_step=(epoch + 1),
            )
            tensorboard_obj.add_scalars(
                "test_accuracy_percent/",
                {config["nick"]: mean_test_accuracy.value() * 100},
                global_step=(epoch + 1),
            )
        ...
        if is_best_so_far:
            store_checkpoint(
                output_dir, "best.checkpoint", model, epoch, mean_test_accuracy.value()
            )
    ...
    if results_dict != None:
        results_dict["best_accuracy"] = best_accuracy_so_far.value()
...
def get_model(config, device=-1, relu_inplace=True):
    ...
    model = {
        "vgg11_nobias": lambda: models_n.VGG(
            "VGG11",
            num_classes,
            batch_norm=False,
            bias=False,
        ...
        "vgg11_half_nobias": lambda: models_n.VGG(
            "VGG11_half",
            num_classes,
            batch_norm=False,
            bias=False,
        ...
        "vgg11_doub_nobias": lambda: models_n.VGG(
            "VGG11_doub",
            num_classes,
            batch_norm=False,
            bias=False,
        ...
        "vgg11_quad_nobias": lambda: models_n.VGG(
            "VGG11_quad",
            num_classes,
            batch_norm=False,
            bias=False,
...
def get_retrained_model(
    ...
    if args.retrain_lr_decay_epochs is not None:
        config["optimizer_decay_at_epochs"] = [
            int(ep) for ep in args.retrain_lr_decay_epochs.split("_")
        ]
...
if __name__ == "__main__":
    config = dict(
        ...
        model="resnet18",
        optimizer="SGD",
        optimizer_decay_at_epochs=[30, 60, 90, 120, 150, 180, 210, 240, 270, 300],
    ...
    output_dir = "./resnet18_cifar10_new"  # Can be overwritten by a script calling this
    gpu_id = 0
